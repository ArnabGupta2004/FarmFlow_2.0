...
export default function Seller() {
  ...
  const handleOpenChat = (deal) => {
    ...
    if (!farmer_id || !uniqueID) {
      toast.error(t("seller.missingIds"));
      return;
    }
  ...
  return (
    <div className="seller-wrapper">
      ...
      <div className="seller-header">
        <h1>{fpcName || t("seller.defaultName")}</h1>
        <div className="sub">{t("seller.portal")}</div>
      </div>

      <section className="glass-box">
        <h3>{t("seller.pendingRequests")} ({reqs.length})</h3>
        <div className="card-list">
          {reqs.map((r) => (
            <div key={r.id} className="card">
              <div className="card-title">{r.farmer_name}</div>
              <div className="card-district">{t("seller.region")}: {r.region}</div>
              <div className="card-commodities">{r.crop} • ₹{r.price}</div>

              <button className="connect-btn" onClick={() => accept(r.id)}>{t("seller.accept")}</button>
              <button className="reject-btn" onClick={() => reject(r.id)}>{t("seller.reject")}</button>
            </div>
          ))}
        </div>
      </section>
